...
let URL = "https://teachablemachine.withgoogle.com/models/IZ7GRn80Q/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 27.0 && time <= 29.0 && !poseState.triggered) {
            ...
            case '2':
                ...
                break;
            case '3':
                if ((time >= 124.0 && time <= 126.0 && !poseState.firstWindowTriggered) ||
                ...
                break;
            case '4':
                if (time >= 126.1 && time <= 128.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                    if (time >= 138.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/IZ7GRn80Q/");
    }
}
